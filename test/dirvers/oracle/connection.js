...
const { Condition } = require('../../../lib/builder')
const assert = require('assert')
const _ = require('lodash')
const { EventEmitter } = require('events')

const { OperatorMaps, CompareOperators, LogicJoinOperators, LogicNotOperator } = require('../../../lib/constants')
...
class Connection extends EventEmitter {
  ...
  /**
   * 执行存储过程
   * @param {string} sp 存储过程名称
   * @param  {any[]} params 参数列表
   */
  async execute(sp, ...params) {
    if (_.isArray(params[0]) && params.length === 1) {
      params = params[0]
    }
    const dbParams = []
    const sql = `EXEC ${sp}(${params.map(p => this._appendParam(dbParams, p)).join(', ')})`
    const res = await this._query(sql, dbParams)
    return res
  }

  /**
   * 关闭所有连接
   * @param {integer?} drainTime 等待连接关闭时间，如果传0，则会立即关闭连接
   */
  async close(drainTime) {
    if (this._hasTrans) {
      throw new Error('事务尚未提交！')
    }
    await this._connection.close(...arguments)
  }
