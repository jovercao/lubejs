 ...
 * 类型断言
 * @param {*} expr 要判断的值
 * @param {Function[]} expectTypes 类型列表
 * @param {*} message 错误消息
 */
...
/**
 * @typedef {new(...expr)=>any} Constructor 类构造函数
 * 推导表达式类型,保证表达式类型符合预期
 * 当表达式是Quoted类型时，会依据Quoted.expr进行类型判断
 * @param {*} expr 表达式，可以是字符串，常量等
 * @param {(expr)=>any} factory 当不是期望类型时的AST类型构造函数
 * @param {Constructor} ExceptType 期望的AST类型构造函数
 * @param {(Constructor|null|undefined|StringConstructor|NumberConstructor|BooleanConstructor|BigIntConstructor|DateConstructor)[]} compatibleTypes 可接受的兼容类型
 ...
 * @param {Condition|Object} condition 条件表达式
 * @returns {Condition}
 */
function ensureCondition(condition) {
  ...
  return Condition.and(Object.entries(([key, value]) => Condition._compare(key, value)))
