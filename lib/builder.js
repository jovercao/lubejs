...
function astEntry(ast, defaultType) {
  assert(_.isPlainObject(ast), 'involid argument ast')
...
function makeTableProxy(table) {
  ...
  return new Proxy(table, {
    get(target, prop) {
      ...
      return new Field(prop, name)
    }
  })
}

function makeNamedTableProxy(namedTable) {
  const name = namedTable[1] || namedTable.$named[1]
  return new Proxy(namedTable, {
    get(target, prop) {
      if (Object.prototype.hasOwnProperty.call(target, prop) || target[prop] !== undefined) {
        return target[prop]
      }
 ...
 * @extends {AST}
 */
class Sets extends Identifier {
  as(alias) {
    return makeNamedTableProxy(sql.named(this, alias))
  }
}
 ...
 * 查询条件类型
 */
class LogicExpression extends AST {
  constructor(context) {
    super()
...
class SelectStatement extends Statement {
  constructor({ table, joins, columns, where, orders, groups }) {
    super()
    this._joins = []
    this._orders = []
