...
function makeAutoFieldTableProxy(table) {
  ...
  return new Proxy(table, {
    ...
    set(target, prop, value) {
      ...
      target[prop] = new AssignStatement(new Field(prop, tableAst), value)
      return target[prop]
    }
  })
}
...
class UpdateStatement extends Statement {
  ...
  set(sets) {
    ...
    if (_.isPlainObject(sets) && arguments.length === 1) {
      this._sets = Object.entries(sets).map(([key, value]) => new AssignStatement(sql.field(key), ast(value)).ast)
      return this
    }
