...
class Expression extends AST {
    ...
    as(alias) {
        ...
        return util_1.makeProxiedIdentifier(identifier);
    }
    /**
     * 算术运算 +
     * @param left 左值
     ...
     * @param name
     */
    static proxiedIdentifier(name) {
        return util_1.makeProxiedIdentifier(util_1.ensureIdentifier(name));
    }
    /**
...
class Join extends AST {
    ...
    constructor(table, on, left = false) {
        ...
        * @type {Table}
        */
        this.table = util_1.ensureIdentifier(table);
        /**
         * 关联条件
        * @type {Condition}
        */
...
class Identifier extends Expression {
    ...
    constructor(name, parent, type = constants_1.SQL_SYMBOLE.IDENTIFIER) {
        ...
        this.name = name;
        if (parent) {
            this.parent = util_1.ensureIdentifier(parent);
        }
        else {
            this.parent = null;
        }
     ...
     */
    dot(name) {
        return util_1.makeProxiedIdentifier(new Identifier(name, this));
    }
    any() {
...
class Invoke extends Expression {
    ...
    constructor(func, args) {
        ...
        this.func = util_1.ensureIdentifier(func);
...
class When extends AST {
    constructor(expr, then) {
        ...
        if (util_1.isJsConstant(expr)) {
            this.expr = util_1.ensureConstant(expr);
        }
        else {
            this.expr = util_1.ensureCondition(expr);
        }
        this.value = util_1.ensureConstant(then);
    }
}
exports.When = When;
/**
...
class Fromable extends Statement {
    ...
    from(...tables) {
        ...
        this.tables = tables.map(table => util_1.ensureIdentifier(table));
        return this;
    }
    /**
     * 表联接
...
class Select extends Fromable {
     ...
     */
    as(alias) {
        return util_1.makeProxiedIdentifier(new Alias(this.quoted(), alias));
    }
}
...
class Insert extends Statement {
    ...
    _fields(...fields) {
         ...
         * 字段列表
         */
        this.fields = fields.map(p => util_1.ensureIdentifier(p));
        return this;
    }
    values(...args) {
...
class Execute extends Statement {
    constructor(proc, args) {
        ...
        this.proc = util_1.ensureIdentifier(proc);
