...
class SchemaObject extends AST {
...
class Condition extends AST {
  ...
  and(condition) {
    condition = mormalizeWhere(condition)
 ...
 * @extends {BinaryExpression}
 */
class Modular extends BinaryExpression {
  constructor(left, right) {
...
class SelectStatement extends Statement {
  ...
  columns(columns) {
    ...
    if (_.isPlainObject(columns) && arguments.length === 1) {
      this._columns = Object.entries(columns).map(([name, exp]) => new Column(exp, name))
      return this
    }
  ...
  orderby(orders) {
    ...
    if (arguments.length === 1 && _.isPlainObject(orders)) {
      ...
      const [exp, direction = ASC] = arguments
      orders = [
        [exp, direction]
      ]
    }
    ...
    this._orders.push(...orders.map(([exp, dir]) => {
  ...
  groupby(groups) {
    ...
    groups.forEach(exp => this._groups.push(exp))
    return this
  }
  ...
  get ast() {
    ...
    return {
      [$select]: {
        ...
        columns: this.fields.map(col => ast(col)),
        table: ast(this._table),
        ...
        where: ast(this._where),
        orders: this.orders.map(([exp, dir]) => [ast(exp), dir]),
        groups: this.groups.map(exp => ast(exp)),
        having: ast(this._having),
        offset: this._offset,
        limit: this._limit
      }
    }
   ...
   */
  union(sql) {
    return new UnionStatement(this, sql)
  }

  unionAll(sql) {
    return new UnionStatement(this, sql).distinct()
  }
}

class UnionStatement extends Statement {
  constructor(...selectStatuments) {
    super()
    this._statements = selectStatuments.map(p => ast(p))
  }
  ...
  get ast() {
    assert(this._statements, 'statements is not allowed empty.')
    return {
      [$union]: {
        distinct: this._distinct || false,
