...
function mormalizeCondition(obj) {
  ...
  const parser = function([key, value]) {
    ...
    const [op, exp] = astEntryByName(value, $eq)
    // 比较运算
    return {
      [op]: [
        new Field(key).ast,
...
class Condition extends AST {
   ...
   * @memberof Condition
   */
  static join(conditions, joinOperator = $and) {
    assert(_.isArray(conditions) && conditions.length > 1, 'Conditions must type of Array & have two or more elements.')
    assert(joinOperator === $and || joinOperator === $or, 'joinOperator must $and or $or.')
    return new Condition({
      [joinOperator]: conditions.map(p => ast(p))
    })
  }
