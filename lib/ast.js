...
class Expression extends AST {
    ...
    static identity(...names) {
        ...
        return names.reduce((previous, current) => {
            if (!previous)
                return new Identity(current);
            return new Identity(current, previous);
        });
    }
    /**
     * 调用表达式
...
class Insert extends Statement {
     ...
     * 构造函数
     */
    constructor(table, fields /*options?: InsertOptions*/) {
    ...
    values(...args) {
        ...
        if (args.length === 1 && args[0] instanceof Select) {
            this.rows = args[0];
            return this;
        }
        if (_.isArray(args[0])) {
        ...
        if (args.length === 1 && !this.fields) {
            this._fields(...Object.keys(args[0]));
        }
        if (!this.fields) {
