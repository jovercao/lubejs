...
const { assert, astType, astEntry } = require('./util')
const {
  Types,
...
class SysType extends AST {
  constructor(type) {
    super()
    assert(Types.includes(type), `type must one of ${Types.join(',')}`)
    this._type = type
  }
...
class SelectStatement extends Statement {
  ...
  /**
   * 内联接
   * @param {string|Table|FnCallStatement} table
   * @param {Condition} on
   * @param {string} alias
   * @memberof SelectStatement
   */
...
class UpdateStatement extends Statement {
  ...
  set(sets) {
    ...
    this._sets = sets.map((set) => {
      assert(_.isArray(set), 'set element must typeof array')
...
class Parameter extends Expression {
  constructor(name, value, direction) {
