...
const { Operators, NULL } = require('./constants')

const logicOperators = ['$and', '$or']
const notOperator = '$not'
const compareOperators = ['$eq', '$uneq', '$gt', '$lt', '$gte', '$lte', '$in', '$not', '$like', '$is']
  ...
  _compileWhere(where, params) {
    ...
    return Object.entries(where).map(([key, item]) => {
      ...
      if (_.isObject(item)) {
        ...
        assert(compareOperators.includes(keys[0]), `比较表达式运算符${keys[0]}不正确，运算符必须是${compareOperators.join(',')}中的一种`)
