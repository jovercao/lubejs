...
const { ASC, DESC, OUTPUT, $and, $or, $not, $is, $in, $eq, $neq, $gt, $gte, $lt, $lte, $like, $add, $sub, $mul, $div, $mod, $param, $var, $fn, $proc, $table, 
// $sysfn,
// $exec,
$datatype, $fncall, $syscall, $select, $update, $insert, $delete, $iif, $execute, $assign, $const, $field, $column, $quoted, $exists, SqlSymbolNames } = require('./constants');
const OperatorMapps = {
    [$and]: 'AND',
    [$or]: 'OR',
    [$not]: 'NOT',
...
class Compiler {
    ...
    _compile(ast, params, defaultType, excepts, errMsg, handler) {
        ...
        switch (type) {
            ...
            case $execute:
                ...
                sql = `EXEC ${this._compile(require('./builder').output('_ReturnValue_', require('./constants').NUMBER).ast, params)} = ${this._compile(item.proc, params, $proc, [$proc])} ${item.params.map(p => this._compile(p, params, $const, [$const, $var, $param], null, (txt, tp, it) => {
    ...
    _compileSetsName(ast, params, defaultType, excepts) {
        if (!excepts) {
            excepts = [$table, $fncall];
        }
        const [type, value] = astEntry(ast, defaultType);
    ...
    _compileSelectStatement(select, params) {
        ...
        if (having) {
            sql += ' HAVING ' + this._compileCondition(having, params);
        }
        if (orders) {
