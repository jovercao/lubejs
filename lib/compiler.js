...
class Compiler {
  ...
  _parseConstant(value) {
    ...
    if (_.isDate(value)) {
      return "CONVERT(DATETIMEOFFSET, '" + moment(value).format('YYYY-MM-DDTHH:mm:ssZ') + "')"
    }
    if (_.isBuffer(value)) {
  ...
  _compile(ast, params, defaultType, excepts, errMsg, handler) {
    ...
    switch (type) {
      ...
      case $table:
      // case $view:
        sql = item.alias ? this._parseObject(item) + ' AS ' + this._quoted(item.alias) : this._parseObject(item)
        break
      case $proc:
        sql = this._parseObject(item)
        break
      case $execute:
  ...
  _compileInsertStatement(insert, params) {
    ...
    const [type, item] = astEntry(table, $table)
    checkType(type, [$table])
    let sql = `INSERT INTO ${this._parseObject(item)}`
