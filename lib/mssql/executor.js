...
class Executor extends EventEmitter {
  ...
  _compile(ast, params, defaultType, excepts, errMsg) {
    ...
    switch (type) {
      ...
      case '$syscall':
        return `${SysfnMapper[value.name]}(${value.params.map(p => this._compileExpression(p))})`
      case '$table':
      case '$view':
  ...
  _compileField(field) {
    ...
    if (_.isPlainObject(field)) {
      let sql = field.name === '*' ? field.name : this._quoted(field.name)
    ...
    return this._compileConstant(field)
  }
  ...
  _compileSelectStatement(select, params) {
    ...
    sql += `${(columns && columns.length > 0) ? this._compileColumns(columns, params) : '*'}`
    if (table) {
  ...
  _compileInsertStatement(insert, params) {
    ...
    return `INSERT INTO ${this._compile(table, params, '$table', ['$table'])}(${fields.map(p => this._compileExpression(p, params, '$field'))}) VALUES(${values.map(p => this._compileExpression(p, params, '$const')).join(', ')})`
  }
  ...
  async insert(table, row) {
    ...
    if (_.isArray(row)) {
      if (row.length > 1) {
        ...
        // 如果未开启事务，则自动开启事务
        if (!this._isTrans) {
          await this._driver.warp.trans(async (exe) => {
            for (const r of row) {
              const count = await exe.insert(table, r)
              rowsAffected += count
            }
          })
        } else {
