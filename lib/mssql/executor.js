...
const { astEntry, Statement, AST } = require('../builder')
...
class Executor extends EventEmitter {
  ...
  _compile(ast, params, defaultType, excepts, errMsg) {
    let [type, value] = astEntry(ast, defaultType)
    if (excepts) {
  ...
  _compileInsertStatement(insert, params) {
    ...
    VALUES(${values.map(p => this._compileExpression(p, '$const')).join(', ')})`
  }

  _compileUpdateStatement(update, params) {
    let { table, values, where } = update
    assert(table && values, 'table and values is required by update statement')
    const sql = `UPDATE ${this._compile(table, params, '$table', ['$table'])} SET ${Object.entries(values).map(([field, value]) => `${this._quoted(field)} = ${this._compileExpression(value, params, '$const')}`).join(', ')}`
    if (where) {
      sql += ' WHERE ' + this._compileCondition(where, params)
    }
    return sql
