...
class Lube extends events_1.EventEmitter {
    constructor(provider, ployfill, options) {
        super();
        util_1.assert(provider, 'provider is required.');
        util_1.assert(_.isFunction(provider.query), 'provider must provide function: query');
        util_1.assert(_.isFunction(provider.beginTrans), 'provider must provide function: beginTrans');
        this._provider = provider;
        const compiler = new parser_1.Parser(ployfill, options);
        ...
        this._executor = new executor_1.Executor(query, compile, ployfill.properParameter);
    ...
    async trans(handler, isolationLevel) {
        ...
        const executor = new executor_1.Executor(query, this._compile);
...
async function connect(config) {
    if (typeof config === 'string') {
        const url = new url_1.URL(config);
